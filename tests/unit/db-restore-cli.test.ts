...
// The main() tests below drive the real control flow with the deferred db/store
// imports mocked, so no real db, network, or snapshot file is ever touched.
// vitest matches these mocks to the script's own imports by resolved path.
...
import { restoreCases } from "../../lib/storage/pg-backup";
