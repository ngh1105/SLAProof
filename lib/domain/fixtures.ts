...
import type { EvidenceItem, SlaCase } from "./types";

function evidence(item: Omit<EvidenceItem, "hash">): EvidenceItem {
  return {
    ...item,
    hash: hashEvidence(item.submittedExcerpt),
  };
}
...
export function getDemoCase(caseId: string): SlaCase | undefined {
  return demoCases.find((slaCase) => slaCase.id === caseId);
}
